...
extern void Turn180CommandInit(Turn180Command_t *cmd, uint32_t turn_direction, uint32_t turn_type, float turn_speed, float rev_speed, uint32_t reverse_drive_time, Command_t *next)
{
    ...
    DriveOpenLoopCommandInit(&cmd->rev_drive_cmd, rev_speed, rev_speed, (Command_t*)&cmd->rev_drive_delay_cmd);
    DelayCommandInit(&cmd->rev_drive_delay_cmd, reverse_drive_time, (Command_t*)&cmd->turn180_cmd);
    TurnCommandInit(&cmd->turn180_cmd, turn_direction, 1, turn_type, turn_speed, turn_speed, NULL);
