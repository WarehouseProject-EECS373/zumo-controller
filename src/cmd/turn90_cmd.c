...
static TurnTypeDirectionCfg_t turn_type_dir_cfg[2][2] = {
    {
        {TURN_DIR_LEFT, REFARR_LEFT_SENSOR_ENABLE},
...
{
    cmd->base.on_Start = Turn90CommandStart;
    cmd->base.on_Message = Turn90CommandOnMessage;
    cmd->base.on_End = Turn90CommandOnEnd;
    ...
    LineFollowCommandInit(&cmd->lf_cmd, turn_cfg.mode, 1, 0.0, 0, (Command_t*)&cmd->post_turn_lf_cmd);
    
    uint8_t post_turn_lf_mode = REFARR_DRIVE_CTL_ENABLE | REFARR_LEFT_SENSOR_ENABLE | REFARR_RIGHT_SENSOR_ENABLE;
    LineFollowCommandInit(&cmd->post_turn_lf_cmd, post_turn_lf_mode, post_turn_intersection_count, LINE_FOLLOW_MAX_BASE_VELOCITY, OPEN_LOOP_TURN_DELAY_MS, NULL);
...
{
    Turn90Command_t *tcmd = (Turn90Command_t*)cmd;
